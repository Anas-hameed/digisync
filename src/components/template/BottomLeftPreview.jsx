...
import fonts from '~/constants/fonts/bottomLeft';


const BottomLeftPreview = ({ data, index1, setIsOpen }) => {
	const navigate = useNavigate();
	const { bottomLeftImage,setImage,prompt, posterText, Title, Promotion, Contact, Description, setFont, index, setSelectedPoster } = usePosterContent();
	const handleClick = () => {
		console.log("edit");
		setFont([fonts[index1]]);
		setSelectedPoster(index1);
		setImage(bottomLeftImage);
		...
		<>
			<div className="z-0 hover:opacity-90 relative group">
				<div className="w-[500px] h-[500px] relative overflow-hidden rounded-lg shadow-lg cursor-pointer">
					...
					<div className="absolute top-0 left-0 px-6 py-6">
						<div className="relative">
							<h4 style={{ "fontFamily": `${fonts[index1].headingfont}` }}
								className={`mb-3 text-[30px] font-bold tracking-tight text-gray-100 w-full max-w-[400px] pl-2 border-2 px-4 py-2`}
							>
								{Title}
							</h4>
					...
					<div className="absolute top-[36%] left-0 px-6">
						...
						<p style={{ "fontFamily": `${fonts[index1].paragraphfont2}` }}
							className={`text-sm text-gray-100 mt-[4px]`}>{Description} </p>
					</div>
					<div className="absolute bottom-4 right-0 px-6">
						<div className="relative flex flex-col items-start leading-[24px]">
							<h4 style={{ "fontFamily": `${fonts[index1].paragraphfont3}` }}
								className={`text-xs font-bold text-gray-100 max-w-[260px]`}>
								Graphic Generated By AI
							</h4>
							<p style={{ "fontFamily": `${fonts[index1].paragraphfont3}` }}
								className={`text-[10px] text-gray-100 max-w-[260px]`}>
								prompt: {prompt}
							</p>
						</div>
...
export default BottomLeftPreview;