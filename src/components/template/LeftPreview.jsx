...
const LeftPreview = ({ data, index1, setIsOpen }) => {
	console.log(index1)
	const navigate = useNavigate();
	const { leftImage,setImage,prompt, posterText, Title, Promotion, Contact, Description, setFont, index, setSelectedPoster } = usePosterContent();
	const handleClick = () => {
		console.log("edit");
				...
				<div className="w-[500px] h-[500px] relative overflow-hidden rounded-lg shadow-lg cursor-pointer">
					...
					<div className="absolute top-0 right-0 px-6 py-12">
						<div className="relative">
							<h4 style={{ "fontFamily": `${fonts[index1].headingfont}` }}
								className={`mb-3 text-[30px] font-bold tracking-tight text-gray-100 w-full max-w-[240px] border-2 p-2`}
							>
								{Title}
							</h4>
							<p style={{ "fontFamily": `${fonts[index1].paragraphfont1}` }}
								className={`max-w-[240px] text-sm leading-normal text-gray-100 pl-2`}>
								{posterText[index]}
							</p>
						</div>
					</div>
					<div className="absolute top-[54%] left-0 px-6">
					...
					<div className="absolute right-0 bottom-0 p-2">
						<div className="flex items-center flex-col">
							...
							<p style={{ "fontFamily": `${fonts[index1].paragraphfont3}` }}
								className={`text-sm font-normal text-gray-100 ml-2`}>{` ${Contact}`}</p>
						</div>
					</div>
				</div>
